          ...
          <div class="col-md-12">
            <div class="text-right invoice-print_btn">
              <button class="btn btn-back-two btn-warning text-white"
                      ...
                      order='default'
                      :endpoint="'https://admin.masterleatherbd.com/sslcommerz/existing-order-pay?order_id='+order_masters.order_id">
                ...
                <div id="content">
                  ...
                  <table class="invoice_product_details ">
                    ...
                    <tr v-for="(order,key) in order_details" :key="key">
                      ...
                      <td>
                        <nuxt-link :to="'/product/'+order.seo_url"> {{ order.name }}</nuxt-link>
                        <!--                        <br> {{ order.sub_title }}-->
                      ...
                      <td>
                        ...
                        {{ money_sign }} {{ Math.round(order.local_purchase_price).toLocaleString() }}
                      </td>

                      <td>
                        {{ money_sign }}
                        {{ Math.round((order.local_selling_price - order.local_purchase_price)).toLocaleString() }}
                      ...
                      <td>
                        ...
                        <span v-else>
                          {{
                            ...
                            Math.round(order_masters.coupon_discount ? order_masters.coupon_discount : 0).toLocaleString()
                          }}
                        </span>
                      </td>
                    </tr>
        ...
        <section slot="pdf-content">
          ...
          <div id="invoice-info" style="padding:5px;margin:20px;">
            <div class="text-center font-weight-bold d-block mb-2">
              প্রোডাক্ট পাওয়ার পর সার্ভিসের জন্য কল করুন: ০১৭৫৭৫ ৭৫৭২৩৫
            </div>
            <table class="table-responsives" style="width:97%;">
              <tr class="invoice-contact-list justify-content-between">
            ...
            <table class="table-responsives product-list-info mt-2" style="width:97%;">
              ...
              <tr v-for="(order,key) in order_details" :key="key">
                ...
                <td>{{ order.product_code }}</td>
                <td> {{ money_sign }} {{ Math.round(order.local_purchase_price).toLocaleString() }}</td>
                <td>
                  {{ money_sign }}
                  {{ Math.round((order.local_selling_price - order.local_purchase_price)).toLocaleString() }}
                  ({{ order.discount ? order.discount : 0 }}%)
                ...
                <td> {{ order.qty }}</td>
                <td>{{ money_sign }} {{ Math.round(order.total_purchase_price).toLocaleString() }}</td>
              </tr>
              <tr>
                <td colspan="11" style="text-align: right;">Sub-Total :</td>
                <td>{{ money_sign }} {{ Math.round(order_masters.total_amount).toLocaleString() }}</td>
              </tr>
              <tr>
                <td colspan="11" style="text-align: right;">Coupon Discount :</td>
                <td>
                                                <span v-if="order_masters.coupon_type == 'Percentage'">
              ...
              <tr>
                <td colspan="11" style="text-align: right;">Delivery Charge :</td>
                <td></td>
              </tr>

              <tr>
                <td colspan="11" style="text-align: right; border-right: 0;"><b>Grand Total :</b></td>
            ...
            <table class="invoice-description" style="width:97%;">
              <tr>
                <td style="text-align: center;" colspan="10">
                  ...
                  <p>You can contact us through <b>info@masterleatherbd.com</b> and one of our concerns will contact
                    you immediately.</p>
                </td>
              </tr>
...
export default {
  ...
  methods: {
    ...
    //       event: "purchase",
    //       ecommerce: {
    //           transaction_id: self.order_masters.order_id,
    //           affiliation: "Google Merchandise Store",
