        ...
        <div class="container">
            <div class="interiors-full-wrapper">
                 <div class="section-title" >
                            <h2 class="title" >{{$store.state.common.INTERIORS.category?$store.state.common.INTERIORS.category.name: parentCat.name}}</h2>
                        </div>
                <div class="interior-post-content">

                    <div class="post-list" :class="[loading || $route.query.slug ?'d-none':'']">
                        <nuxt-link v-for="(cat,ckey) in category" :key="ckey" :to="`/gallery/${parentCat.seo_url}?slug=`+cat.seo_url" class="interior-product-items active">
...
export default {
  components: { LazyImage, Loading, GalleryModal },
       ...
       async catInteriors(){
               const self = this
                ...
                if(this.$store.state.common.INTERIORS.category){
                    this.currentPath = [{
                        link: `/gallery/${self.parentCat.seo_url}?slug=` + this.$store.state.common.INTERIORS.category.seo_url,
    ...
    async created(){
        ...
        this.parentCat = this.$store.state.common.INTERIOR_CATEGORY.parent

        this.pathList = [{
                link: `/gallery/${self.parentCat.seo_url}`,
                label: self.parentCat.name
        }];
