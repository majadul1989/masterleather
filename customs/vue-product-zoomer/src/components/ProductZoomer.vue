<template>
    <div :class="options.namespace + '-base-container scroller-at-' + options.scroller_position">
      <img :class="[choosedThumb.id=='threesixtydegree' || choosedThumb.id == 'youtube' ?'d-none':'']" @click="openGalleryModal"
        :src="previewImg.url"
      ...
      <div :class="[choosedThumb.id=='youtube' ? '':'d-none']" class="embed-responsive embed-responsive-16by9">
          <iframe frameborder="0"
                  ...
                  marginheight="0"
                   ...
                   :src="'https://www.youtube.com/embed/'+choosedThumb.yt+'?autoplay=0&fs=1&iv_load_policy=3&showinfo=0&rel=0&cc_load_policy=0&start=0&end=0&origin=https://masterleatherbd.com'">
          </iframe>
      </div>



      <div :class="[choosedThumb.id !='threesixtydegree' && view360Init ? 'd-none':'d-anone']">

             <div id="circlr">

                <img v-for="(dimg,key) in degree_images" :key="key" :data-src=" baseUrl() + dimg">
                <div id="loader"></div>
...
import i360Viewer from './i360Viewer'
...
export default {
  ...
  components: {i360Viewer,GalleryModal},
  data() {
    return {
      previewImg: {},
      previewLargeImg: {},
  ...
  watch: {
    choosedThumb: function(thumb) {
      ...
      if(thumb.id == 'threesixtydegree' && this.view360Init == false)
      {
        this.view360Init = true
  ...
  created() {
    ...
    this.galleryImgs = this.normal_size.map((item,index) => {

      return item.url.replace('https://admin.masterleatherbd.com/','')
    })
  ...
  methods: {
    ...
    moveThumbs(direction) {
      let moveThumb = direction !== "backward" ? this.thumbs[0] : this.thumbs[this.thumbs.length - 1],
          allThumb = this.thumbs.filter((item) => {
            return moveThumb.id != item.id
          })

          console.log(moveThumb,direction)

      if (direction === "backward") {
          this.thumbs = [moveThumb, ...allThumb];
      } else {
        this.thumbs = [...allThumb, moveThumb];
      }
